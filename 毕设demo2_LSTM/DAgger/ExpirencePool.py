...
class ExperiencePool:
    def __init__(self, n_features, n_maxexps, n_clusters, n_timesteps):
        self.n_features = n_features
        self.n_timesteps = n_timesteps
        self.n_exps = 0
        self.n_maxexps = n_maxexps
        self.n_clusters = n_clusters
        self.memory = np.zeros((self.n_maxexps, n_features*n_timesteps))
        self.memory_iter = 0
        self.is_build = False
    ...
    def updateMeans(self, data, updateMode):
        ...
        if center_dis > self.maxMeanDis and updateMode == 1:
            center_id = self.n_clusters
            self.n_clusters += 1
            self.cluster_mean = np.append(self.cluster_mean, data.reshape(1, data.shape[1]), axis=0)
            for center in self.cluster_mean:
    ...
    def Random_Sample(self, batch_size):
        ...
        return self.memory[sample_index, :].reshape(batch_size, self.n_timesteps, self.n_features)
    ...
    def maxEntropy_Sample(self, batch_size, model):
        ...
        def calculate_Entropy(model, data):
            ...
            return res.item()
        for data_id in range(self.n_maxexps):
            entropy = calculate_Entropy(model, self.memory[data_id].reshape(1, self.n_timesteps, self.n_features))
        ...
        while i < batch_size:
            ...
            batch_data.append(self.memory[select_data[1], :])
            i += 1
        return np.array(batch_data).reshape(batch_size, self.n_timesteps, self.n_features)
    ...
    def LossWeighted(self, batch_size):
        ...
        if not self.is_lossNet:
            ...
            self.is_build = True
        for data_id in range(self.n_maxexps):
            loss = self.LossPred.pred(torch.FloatTensor(self.memory[data_id].reshape(1, self.n_timesteps, self.n_features)))
            mean_loss = torch.mean(loss).item()
            heapq.heappush(heap, (-mean_loss, data_id))
        i = 0
        loss = 0
        while i < batch_size:
            select_data = heapq.heappop(heap)
