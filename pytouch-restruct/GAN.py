...
for epoch in range(num_epoch):  # 进行多个epoch的训练
    for i, (img, _) in enumerate(dataloader):
        ...
        real_label = Variable(torch.ones(num_img))  # 定义真实的图片label为1 [1,1,1,1,1,1]
        fake_label = Variable(torch.zeros(num_img))  # 定义假的图片的label为0 [0,0,0,0,0,0]
        ...
        d_loss_real = criterion(real_out, real_label)  # 得到真实图片的loss [x1x2x3x4x5] -(logx1x2x3x4x5)
        ...
        d_loss_fake = criterion(fake_out, fake_label)  # 得到假的图片的loss -(log(1-z1)(1-z2)(1-z3))
        ...
        g_loss = criterion(output, real_label)  # 得到的假的图片与真实的图片的label的loss -log(z1z2z3z4z5)
        # bp and optimize
